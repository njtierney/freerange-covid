...
vic_caption <-
  glue(
    "Data from the DHHS; analysis by http://freerangestats.info. Last updated {Sys.Date()}."
  )
...
forecast_cases <- estimates_vic$plots$reports$data %>%
  mutate(across(where(is.numeric), function(x) {
    x / latest_positivity_correction
  })) %>%
...
fc_plot_vic <- forecast_cases %>%
  ...
  annotate(
    "text",
    x = latest_fc$date + 1,
    y = c(latest_fc$bottom, latest_fc$top),
...
svg_png(
  fc_plot_vic,
  "../img/covid-tracking/victoria-latest-fc",
  h = 5,
  w = 10
)
...
svg_png(
  fc_plot_vic,
  "../_site/img/covid-tracking/victoria-latest-fc",
  h = 5,
  w = 10
)
